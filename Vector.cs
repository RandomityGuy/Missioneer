...
namespace Missioneer
{
    public struct Vector : IEquatable<Vector>
    {
        ...
        {
            X = Convert.ToSingle(xyz.GetWord(0));
            Y = Convert.ToSingle(xyz.GetWord(1));
            Z = Convert.ToSingle(xyz.GetWord(2));
        }
        ...
        public static float Angle(Vector a, Vector b)
        {
            var an = a.Normalized;
            var bn = b.Normalized;
            if (Math.Abs(an.SquareLength) < 0.0000001f
                || Math.Abs(bn.SquareLength) < 0.0000001f)
            {
                return 0f;
            }
        ...
        {
            return Math.Abs(X - other.X) < 0.0000001f
                && Math.Abs(Y - other.Y) < 0.0000001f
                && Math.Abs(Z - other.Z) < 0.0000001f;
        }
